...
|---|---|---|---|---|---|
| <a id="d-001"></a>`D-001` | `proposed` | Нужны ли direct per-daemon ports после unified hub? | оставить только как диагностический/compatibility mode, по умолчанию clients направлять в единый ingress | до WP-1 | — |
| <a id="d-002"></a>`D-002` | `proposed` | Поддерживать ли MCP `2025-03-26`? | удалить, если нет подтверждённого обязательного клиента; иначе реализовать batch | до WP-4 | — |
| <a id="d-003"></a>`D-003` | `proposed` | Как изолировать Serena? | dynamic project pool по canonical root | до WP-3 | — |
| <a id="d-004"></a>`D-004` | `proposed` | Нужен ли owner token? | да, required для stable 1.0 | до WP-1 | — |
| <a id="d-005"></a>`D-005` | `proposed` | Какой defect registry canonical? | GitHub Issues/Projects либо schema-validated files, но не оба независимо | немедленно | — |
| <a id="d-006"></a>`D-006` | `proposed` | Поддержка macOS в 1.0? | только при native CI; иначе официально preview вне 1.0 GA matrix | до RC | — |
| <a id="d-007"></a>`D-007` | `proposed` | Как публиковать npm? | OIDC trusted publishing через protected environment | до WP-9 | — |
| <a id="d-008"></a>`D-008` | `proposed` | Как обрабатывать bidirectional shared server? | isolate per session/client до готовности полноценного reverse router | до WP-2 | — |
| <a id="d-009"></a>`D-009` | `proposed` | Общий max MCP message size | default 16 MiB, per-manifest lower override, global hard ceiling | до WP-1 | — |
| <a id="d-010"></a>`D-010` | `proposed` | Какой результат lock-release failure? | committed-but-audit-uncertain, без автоматического replay | до WP-6 | — |
...
| <a id="d-017"></a>`D-017` | `proposed` | Как безопасно удалять бывшие client-direct процессы после удаления конфигурации? | Только по persisted former signature с TTL, exact executable/argv/identity proof и fail-closed отказом; process-name kill запрещён. | до реализации DELTA-003 / WP-08 | — |

# Servers matrix revamp: LSP-bridge integration + per-daemon env overlay — Implementation Plan (v5)
...
| Windows reparse-point pattern | The reference file at `hub_mcp_state_dacl_windows.go:85-99` uses `windows.CreateFile(parentW, FILE_LIST_DIRECTORY\|READ_CONTROL\|SYNCHRONIZE, FILE_SHARE_READ\|FILE_SHARE_WRITE\|FILE_SHARE_DELETE, nil, OPEN_EXISTING, FILE_FLAG_OPEN_REPARSE_POINT\|FILE_FLAG_BACKUP_SEMANTICS, 0)` on a **PARENT DIRECTORY**. The directory-listing access flags are dir-specific; for a REGULAR FILE (the overlay), the implementer uses file-read flags instead: `GENERIC_READ` + `FILE_SHARE_READ` + the same `FILE_FLAG_OPEN_REPARSE_POINT\|FILE_FLAG_BACKUP_SEMANTICS` flag combo. The key reusable parts are: (1) the flag combo (SET on `dwFlagsAndAttributes` to refuse reparse-point following), (2) the attribute-refusal idiom at `:187-193` (`bhfi.FileAttributes & windows.FILE_ATTRIBUTE_REPARSE_POINT != 0` → reject). **Defense-in-depth upgrade (optional):** open parent dir with directory flags (as at `:85-99`), then open the child file relative to the parent handle via `ntOpenRelative` (`:157-165`). Use the simpler direct-file form for v0.5.x; upgrade later if the threat model tightens. Returns `windows.Handle`, NOT `*os.File` — the implementer writes a thin reader. | `internal/api/hub_mcp_state_dacl_windows.go:85-99,157-165,187-193` |
...
**Test contract:** Each task above that emits an event MUST add a unit test verifying the event fires — assertion pattern is reading the hub-mcp.log file after the action and grepping for the expected `event` string (see `internal/api/hub_mcp_log_redaction_test.go:97-103` for the canonical pattern). **There is NO `SetHubMcpEventSink` or other mock-sink API** — see catalog discipline rule at top of plan. Total: ~7 new event-fire assertions woven into the relevant tasks' tests.
...
2. **`ServerRow` type extended in `types.ts` (lines 98-105, NOT `routing.ts`)** with `legacyConflict?: Record<string, ClientEntry>` (camelCase per TS conventions; matches the snake_case JSON via the propagation helper in `routing.ts`).
3. `collectServers` (in `routing.ts:130-137`) propagates `e.legacy_conflict` → `row.legacyConflict` on the produced `ServerRow`.
...
7. **`ipcDispatchDeps` MUST be extended** (prescriptive — not optional). The struct at `internal/cli/supervise.go:257-270` currently has `{stateDir, events, runtimeTracker, reconcileReady, intentFilesLoaded, gracefulInProgress, triggerGracefulExit}`. Add these new fields in a small commit BEFORE this task lands (or fold into this task as its first step):
   ...
   Wire location: `ipcDispatchDeps` is constructed at `supervise.go:512`; the production `spawnFn`/`terminateFn` closures are created later at `supervise.go:578-584`. **Preferred wiring: option (b) late-binding** — keep deps construction at `:512` (preserves IPC accept-loop startup ordering — IPC is available before spawn fires) and let `respawnDaemon` resolve its target through a mutex-protected pointer or `sync.Once`-set function field that the supervisor assigns AFTER `:584` once the production closures exist. The existing `triggerGracefulExit` field at `:512-526` is itself early-constructed over a pre-existing channel (it does NOT use this late-binding pattern); `respawnDaemon` is the FIRST late-bound field in this struct. Option (a) — defer deps construction past `:584` — also works but pushes IPC accept-loop later, changing observable startup ordering; only pick (a) if IPC-after-spawn ordering is explicitly desired.

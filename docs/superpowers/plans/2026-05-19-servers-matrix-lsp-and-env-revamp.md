...
7. **`ipcDispatchDeps` MUST be extended** (prescriptive — not optional). The struct at `internal/cli/supervise.go:257-270` currently has `{stateDir, events, runtimeTracker, reconcileReady, intentFilesLoaded, gracefulInProgress, triggerGracefulExit}`. Add these new fields in a small commit BEFORE this task lands (or fold into this task as its first step):
   ...
   Wire location: `ipcDispatchDeps` is constructed at `supervise.go:512`; the production `spawnFn`/`terminateFn` closures are created later at `supervise.go:578-584`. **Preferred wiring: option (b) late-binding closure** — keep deps construction at `:512` (preserves IPC accept-loop startup ordering — IPC is available before spawn fires) and have `respawnDaemon` capture references that get populated after `:584` (similar to the existing `triggerGracefulExit` late-binding pattern at `:512-526`). Option (a) — defer deps construction past `:584` — also works but pushes IPC accept-loop later, changing observable startup ordering; only pick (a) if IPC-after-spawn ordering is explicitly desired.

...
- **Phase C** — Stop auto-installing the `\mcp-local-hub-watchdog` scheduled task (`setup.go`); uninstall it on existing hosts. It actively FIGHTS the supervisor every 5 min — most urgent legacy. Depends on A. **Add minimal supervisor-liveness recovery BEFORE C/D delete the watchdog (§15 P1-b).**
- **Phase D** — Remove the watchdog command + recovery engine (`watchdog.go`, `recovery.go`, `watchdog_state.go`). Depends on C. Gate D on the "kill supervisor mid-session → auto-recovers, zero watchdog code" test.
...
**Phase 3 — legacy removal C→D (Workstream §5; watchdog removal).**
- **#8a precedes C** (see Phase 2) — land the test-port convention + test-tree guard-grep first so C/D's test churn cannot re-introduce a live-band literal.
- **C** — drop `setup.go:471` `InstallWatchdogTask()` auto-install + add idempotent uninstall on existing hosts (most urgent legacy; fights supervisor every 5 min via `recovery.go:299` `IsActiveStop` restart). Depends on A (deployed). **BEFORE C/D delete the watchdog, add a minimal supervisor-liveness recovery (§15 P1-b):** the autostart shim installs the supervisor with `LogonTrigger: true` + `RestartOnFailure: false` (`autostart/windows.go:103-111`), and its own comment names the **watchdog tick** as the ONLY supervisor-death recovery between logons. Deleting the watchdog with no replacement means a mid-session supervisor death (taskkill, OOM, panic, parent Job-close) leaves the fleet down until next logon. Add EITHER a flip to autostart `RestartOnFailure: true` + `StopExisting`, OR one tiny "restart-if-no-`supervisor.lock`-holder" task — NOT the full watchdog.
- **D** — delete `watchdog.go`/`recovery.go`/`watchdog_state.go` + the `Install/UninstallWatchdogTask` API surface. Depends on C. **Gate D on the liveness-recovery acceptance test: "kill the supervisor mid-session → it is back within N minutes with ZERO watchdog code."**
...
**Phase 4-tail — idle-shutdown (#6, §6).** Lands AFTER Phase 4-E (needs the unified intent + the §4/Phase-A corrected stop path — do NOT author a second stop path). `desired=stopped`+`IntentReasonIdle` on the unified intent; 60s in-supervisor sweeper; router clears + 503-retries on wake; GUI-configurable threshold + cold/warm wake-mode. Open verify: serena `.serena/` re-warm cost drives the default.
...
**P1-b — Phase C/D delete the ONLY supervisor-death recovery between logons; add minimal liveness recovery FIRST (fable).** The autostart shim (`autostart/windows.go:103-111`) installs the supervisor with `LogonTrigger: true` + `RestartOnFailure: false`, and its own code comment names the **watchdog tick** as part of supervisor crash recovery. Phase D deletes the watchdog. After D, a mid-session supervisor death (`taskkill /F mcphub.exe`, OOM, panic, parent Job-Object close) leaves the fleet down until the next logon. **Before deleting the watchdog, add a minimal liveness recovery that is NOT the full watchdog:** either flip the autostart task to `RestartOnFailure: true` + `MultipleInstancesPolicy=StopExisting` (the scheduler already supports it), OR keep one tiny scheduled task whose only job is "if no `supervisor.lock` holder is alive, start `mcphub gui`." **Gate D** on: "kill the supervisor mid-session → it is back within N minutes with ZERO watchdog code."

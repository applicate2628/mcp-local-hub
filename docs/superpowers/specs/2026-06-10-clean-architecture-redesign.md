...
**DELETE-LIST TRAPS (architect; a naive "delete the watchdog files" breaks the build).** Two files in the candidate set are NOT cleanly deletable:
- **MUST MIGRATE:** `watchdog_log.go` hosts `splitJSONLines` (`:465`), which `gui_event_log.go:256` depends on (its own comment at `:274` says so). Move `splitJSONLines` to a neutral home (e.g. a new `internal/api/jsonlines.go`) BEFORE deleting `watchdog_log.go`; the rest (`AppendWatchdogLog`/`WatchdogLogEntry`) is watchdog-only and the supervisor uses a SEPARATE `SupervisorEventLog` (`supervisor_events.go:191`).
...
- **Pre-delete MIGRATE (mandatory; a naive delete breaks the build — §15 P1-b):** move `splitJSONLines` out of `watchdog_log.go` (e.g. to `internal/api/jsonlines.go`) — `gui_event_log.go:256` depends on it. Do NOT touch `intent_audit.go` (the supervisor's `SupervisorEventLog` needs `AuditIdentityFieldByteCap`, `intent_audit.go:98` ← `supervisor_events.go:96,273`) — only remove its watchdog-side callers.

...
| Race on cache write | Reuse G4's `writeHubMcpStateFile` helper (codex r1 P1 closure) — flock + atomic rename + DACL re-verify on the parent dir handle. Read path goes through `VerifyHubMcpStateDACL` + `readHubMcpStateFile`. Cache file leaf joins the existing state-name allowlist via `validateStateFileName`. |

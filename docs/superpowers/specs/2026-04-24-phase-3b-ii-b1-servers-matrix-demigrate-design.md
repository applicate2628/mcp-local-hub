...
5. **"demigrate failure on client A gates migrate on client A, dirty prunes successful entries"** — seed three cells: `(A, claude-code)` at `via-hub` (queued demigrate), `(B, claude-code)` at `direct` (queued migrate), `(B, codex-cli)` at `direct` (queued migrate). Stub `/api/demigrate` to 500 for `{servers: [A], clients: [claude-code]}`; leave `/api/migrate` intact.
   ...
   **Pruning assertion:** dirty map after Apply completes contains ONLY the failed `(A, claude-code)` demigrate entry. `(B, codex-cli)` (successful migrate) is pruned. Any `(B, claude-code)` entry that was gated/removed in phase 2 is also pruned.

   **Retry assertion (second Apply click):** un-stub `/api/demigrate` so the retry succeeds. User clicks Apply without re-flipping any cell. Only ONE POST is issued: `/api/demigrate` for `(A, claude-code)`. NO migrate POSTs re-fire (successful phase-1 entries were pruned) and no polluted-backup bug manifests. Assert exactly 1 POST was recorded on the second Apply attempt. **[R6 P1 + R7 P1 + R8 P2 on this memo.]**

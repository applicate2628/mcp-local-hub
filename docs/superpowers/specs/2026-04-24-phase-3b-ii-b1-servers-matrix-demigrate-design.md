...
5. **"demigrate failure on client A gates migrate on client A but not client B"** — seed two rows: `(A, claude-code)` at `via-hub` (queued demigrate), `(B, claude-code)` at `direct` (queued migrate), `(B, codex-cli)` at `direct` (queued migrate). Stub `/api/demigrate` to 500 ONLY for `{servers: [A], clients: [claude-code]}`; leave `/api/migrate` intact. Click Apply. Assert: phase 1 POST `/api/demigrate` fails. Phase 2 POST `/api/migrate` for `(B, codex-cli)` IS sent. Phase 2 POST `/api/migrate` that targeted claude-code is EITHER skipped entirely (no POST recorded for claude-code in phase 2) OR its body has `clients` with `codex-cli` only and `claude-code` removed — the exact shape depends on whether `B` needed both clients or just one. Assert the toolbar shows `Failed: A/demigrate/[claude-code]: 500` and the unaffected migrate completed. Per §4 D4 retry-safety: the failed demigrate on `claude-code` MUST NOT be followed by a migrate POST that would write a polluted latest backup to `claude-code`. **[R6 P1 on this memo.]**

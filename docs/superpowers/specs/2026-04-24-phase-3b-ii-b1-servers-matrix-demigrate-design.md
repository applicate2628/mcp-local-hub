...
6. **E2E:** add 5 scenarios in `servers.spec.ts` (per §6.3: load-demigrate happy path, mixed-Apply demigrate-first ordering, demigrate-failure always-reloads + dirty retains failed entry only, tooltip-copy assertion, per-client-gate retry-safety with dirty-only-failed pruning and second-Apply single-POST assertion); grep + update any `mcphub rollback` tooltip assertion elsewhere. Single commit.

...
### 2.4 `api.NewAPI()` lifecycle — verified no background resources, but `api.go` comment contradicts current use
...
### D3 — Error surface (REVISED per Codex R3 P3 + plan-R4 P1 per-cell refactor)

Reuse the existing `failed: string[]` pattern in `applyChanges`. Each POST represents a single `(server, client, direction)` cell, so the failure label is per-cell:

```ts
failed.push(`${server}/${direction}/${client}: ${body.error ?? resp.status}`);
```

Render `Failed: <joined>` in the toolbar's `apply-msg` span. No inline per-row error (would require row-state plumbing; not a B1 requirement). An earlier draft of this section specified a batched `[{client-list}]` label shape; that was aligned with a batched-POST design that plan-R4 abandoned — see D4's per-cell POST rationale.
...
- `applyChanges` iterates the inner map per-cell — each (server, client, direction) triple fires its OWN POST with a single-element `clients` array: `{servers: [server], clients: [client]}`. An earlier draft of this memo specified batched `(server, direction)` POSTs with multi-element `clients` arrays; that was **rejected by plan-R4** because the GUI adapters `realMigrator` / `realDemigrater` (internal/gui/server.go) collapse per-row failures in `*Report.Failed` into a single aggregated error and the handlers return a single 500. Partial failures inside a batched POST would mark the whole batch failed in the frontend, leaving truly-succeeded rows dirty and replaying them on retry into a now-polluted backup — the R5 sentinel bug again. Per-cell POSTs keep outcome tracking 1:1 with cell state.
...
- **Failure gate per-client: abort migrate phase for any client whose demigrate-phase POST failed.** Rationale: demigrate-before-migrate prevents the polluted-backup bug on the FIRST attempt. But if phase 1 (demigrates) logs a failure for `(server=B, client=claude-code)` and phase 2 (migrates) still runs for `(server=A, client=claude-code)`, `api.MigrateFrom` writes a fresh post-migrate backup to `claude-code` — and the next Apply retry of the failed `demigrate(B, claude-code)` now reads THAT polluted backup, reintroducing the R5 bug across retry. Apply therefore tracks per-client demigrate-phase failure: if any `(server, claude-code)` demigrate in phase 1 fails, every migrate POST in phase 2 strips `claude-code` from its `clients` array (and skips the POST entirely if the resulting `clients` becomes empty). Migrate POSTs for clients with no failed demigrate in phase 1 run normally. A simpler "abort-whole-Apply on any demigrate failure" variant is rejected as unnecessarily blunt — migrate on an unaffected client should still go through. **[R6 P1 on this memo.]**
...
### D6 — Reload-on-every-Apply-completion; PRUNE successful entries from dirty (REVISED per Codex R3 P2 + R7 P1 + R8 P1)

After Apply dispatches, **always** bump `reloadToken` (success or partial failure) to trigger a fresh `/api/scan` + `/api/status` fetch; `servers` state refreshes with authoritative backend routing. **Dirty map update:** clear entirely on full success; on partial failure prune successful entries and retain ONLY failed entries.

Why always reload (revised from "reload-on-success-only"): the Checkbox component at `Servers.tsx:215-224` syncs local `checked` state to `initialChecked` (derived from `server.routing[client]`) via `useEffect([initialChecked])`. Without a post-Apply reload, the `servers` state stays stale — a successful demigrate flipped the backend to `direct` but the frontend still shows `via-hub` as that cell's `routing`. If the user re-toggles the same cell, `initialChecked === true` (stale scan says via-hub) while `nextChecked === false` (user un-checks again), so `toggleCell` enqueues a **second** demigrate against a client that is already direct-routed — the POST fails, UX breaks. Reloading unconditionally keeps every cell's `initialChecked` honest after Apply completes.

Why prune (rather than keep full dirty map): a naive "keep entire dirty map on partial failure" strategy reintroduces R5/R6's polluted-backup bug on retry. Example: `demigrate(B, claude-code)` succeeds in phase 1, `migrate(A, claude-code)` writes a fresh backup in phase 2, `migrate(C, codex-cli)` fails. User clicks Apply. Without pruning, `demigrate(B, claude-code)` replays and reads the NOW-polluted post-migrate backup, falls back to the sentinel, may refuse with "server added after sentinel". With pruning: only `migrate(C, codex-cli)` retries; successful changes are gone.

**Prune semantic — only truly-successful entries, NOT gated entries.** Three per-entry outcomes in an Apply:

1. **Succeeded** (POST fired, got 2xx) → prune from dirty. Retry is a no-op and would reintroduce the polluted-backup bug.
2. **Failed** (POST fired, got non-2xx) → retain in dirty. User can click Apply again to retry.
3. **Gated** (POST never fired because §4 D4's per-client failure gate removed the client from phase 2) → **retain in dirty** (this is the R10 fix). Gated entries represent unfulfilled user intent; the corresponding cell is locally-flipped but backend state is unchanged. Pruning gated entries silently drops intent: after the next reload `initialChecked` stays at the pre-Apply value (the gate prevented any backend change), the Checkbox useEffect is a no-op (local `checked` stays flipped), and the user sees a checked cell with Apply button disabled — UI lies about dirty state. Retaining gated entries keeps Apply enabled so the next click re-attempts the gated migrates once the corresponding demigrate finally succeeds. **[R10 P1 on this memo caught the gated-vs-successful conflation.]**

Implementation shape: `applyChanges` tracks the outcome per entry as a `Map<string, Map<string, "succeeded" | "failed" | "gated">>`; after Apply completes, dirty is rewritten to contain only entries whose outcome is `failed` or `gated`.

Interaction: after the reload, failed cells retain their user-flipped local `checked` state because their `initialChecked` didn't change (backend rejected the change, so `routing` in the new scan matches what it was pre-Apply). The `useEffect([initialChecked])` sync is a no-op for unchanged `initialChecked`. Failed cells stay visually flipped AND stay in dirty map — user can retry Apply or manually un-flip.

Current `Servers.tsx:108-122` reloads only on `failed.length === 0` AND keeps full dirty otherwise. The matrix-wiring task (§7 step 5) therefore introduces TWO new behaviors: unconditional reload, AND success-pruning on partial failure. **[Codex R3 P2 → R7 P1 → R8 P1 on this memo — three rounds of iteration closed the retry-path gaps.]**
...
3. **"demigrate failure surfaces in apply-msg, reload happens, dirty retains failed entry"** — stub `/api/demigrate` to 500 with a generic body. Uncheck a via-hub cell, Apply. Assert the toolbar `apply-msg` contains `Failed:` with the `server/demigrate/[client]` shape from §4 D3. Per §4 D6 (revised R8): reloadToken IS bumped after Apply completes regardless of failure, so `/api/scan` re-runs. The failed cell's routing in the new scan is unchanged (backend rejected the change), so `initialChecked` stays `true` (via-hub) and the Checkbox useEffect is a no-op — the cell's locally-flipped (unchecked) state is preserved. Dirty map retains the failed entry; user can click Apply again to retry. Assert ONE `/api/scan` request was issued after Apply started (to prove unconditional reload), the failed cell's row still shows `via-hub` in the routing column post-reload, and the checkbox shows unchecked (local flipped state intact).
...
5. **"demigrate failure on client A gates migrate on client A, dirty prunes successful entries"** — seed three cells: `(A, claude-code)` at `via-hub` (queued demigrate), `(B, claude-code)` at `direct` (queued migrate), `(B, codex-cli)` at `direct` (queued migrate). Stub `/api/demigrate` to 500 for `{servers: [A], clients: [claude-code]}`; leave `/api/migrate` intact.

   **Phase assertions (first Apply click):** `/api/demigrate` POST fails. `/api/migrate` POST for `(B, codex-cli)` IS sent. `/api/migrate` that would have targeted claude-code is EITHER skipped entirely OR its body has `clients` with `codex-cli` only and `claude-code` removed (per §4 D4 per-client gate). Toolbar shows `Failed: A/demigrate/[claude-code]: 500`. Reload runs (per §4 D6 revised).

   **Prune / retain assertion:** dirty map after Apply completes contains the FAILED `(A, claude-code)` demigrate entry AND the GATED `(B, claude-code)` migrate entry (gated per §4 D4 per-client gate, never POSTed). ONLY the truly-successful `(B, codex-cli)` migrate entry is pruned. Apply button remains enabled because `dirty.size > 0` (two entries retained).

   **Retry assertion (second Apply click):** un-stub `/api/demigrate` so the retry succeeds. User clicks Apply without re-flipping any cell. Phase 1: `/api/demigrate` for `(A, claude-code)` fires and succeeds. Phase 2: `/api/migrate` for `(B, claude-code)` NOW fires (previously gated because the demigrate had failed; retained in dirty per §4 D6 gated-vs-successful rule; no longer gated because the demigrate just succeeded in this second attempt). Assert exactly 2 POSTs on the second Apply — one demigrate + one migrate — in that order. `(B, codex-cli)` migrate does NOT re-fire (pruned on first Apply as a true success). **[R6 P1 + R7 P1 + R8 P2 + R10 P1 on this memo.]**

Target: `internal/gui/e2e/tests/servers.spec.ts` grows from 3 → 8 scenarios (5 new, including the R6-driven retry-safety test). Total E2E suite at current HEAD is **47** (3 shell + 3 servers + 6 migration + 13 add-server + 17 edit-server + 2 dashboard + 3 logs — verified by counting `test(` per spec file). Target after B1: **52**.

**CLAUDE.md accounting is stale.** The committed CLAUDE.md coverage section still says "43 smoke tests total" — written at Task 19 of A2b before the R1 (+3 E2E) and R2 (+1 E2E) Codex-fix commits brought the count to 47. The B1 docs commit must therefore update CLAUDE.md in two steps' worth of delta: catch up to 47 (capture the R1/R2 additions) AND add B1's +5 to land on 52. Plan task for the docs update: one commit that rewrites the coverage section to reflect the current suite, with the 52 final total. [Codex R1 P3 + R6 scenario-5 on this memo.]
...
4. **Servers matrix dirty-shape refactor:** change `dirty` to `Map<string, Map<string, Direction>>`, update `toggleCell` to persist direction, update `applyChanges` to iterate the richer structure and POST per-cell (see §4 D4 — one POST per (server, client, direction) triple, NOT batched per (server, direction)). This is the load-bearing change. Single commit.
...
6. **E2E:** add 5 scenarios in `servers.spec.ts` (per §6.3: load-demigrate happy path, mixed-Apply demigrate-first ordering, demigrate-failure always-reloads + dirty retains failed entry, tooltip-copy assertion, per-client-gate retry-safety with 3-outcome pruning — retain `failed` + `gated`, prune only `succeeded` — and second-Apply fires exactly one demigrate + one migrate in correct order); grep + update any `mcphub rollback` tooltip assertion elsewhere. Single commit.
...
- [x] Codex-review gate — R1 (P1 dirty shape, P2 backup lock myth, P2 api.go contradiction, P3 E2E count) → rev 2; R2 (P2 D4/D7 prune invariant, P2 backup race failure-mode rewrite, P3 CLI parity claim, P3 scope-drift framing) → rev 3; R3 (P2 D6 reload-on-success-only, P3 D3 batch-label shape, P3 R3 empty-clients verified) → rev 4; R4 (P1 typo `direction-clients` → `clients`, P2 NewAPI-not-literally-zero-cost phrasing, P3 `mcphub stop --server` correct CLI shape) → rev 5; R5 (P1 mixed-Apply order: demigrate-first-not-migrate-first, P2 remaining "zero-cost" phrasing in §1/§3.1, P3 `mcphub rollback --client` never existed — removed equivalence claim from §2.3) → rev 6; R6 (P1 retry-safety: per-client gate on migrate phase after demigrate-phase failure to prevent polluted-backup bug across retries, P3 §2.4 heading still said "VERIFIED ZERO-COST" → "verified no background resources") → rev 7; R7 (P1 retry-amplification across already-successful changes: D6 now prunes successful entries from dirty on partial failure so a retry doesn't replay post-polluted-backup demigrates; P3 §3.3/§7 count-drift 4→5 scenarios / 51→52) → rev 8; R8 (P1 successful-cell stale-baseline: D6 now unconditionally reloads after Apply so the Checkbox useEffect baseline stays honest + dirty prune, P3 scenario 5 beefed up to assert dirty-only-failed and second-Apply single-POST retry path) → rev 9; R9 (P3 §7 step 6 description still said "preserves dirty + no reload" — stale from rev 7, corrected to match D6 revised "always-reloads + dirty retains failed entry only") → rev 10; R10 (P1 gated-vs-successful conflation: R7's "prune successful" lumped gated entries with truly-successful ones and silently lost user intent — retry would skip the gated migrate, UI would show checked cell + disabled Apply; D6 now retains both `failed` AND `gated` entries, prunes only `succeeded`; scenario 5 updated with 3-outcome tracking and 2-POST second-Apply assertion) → rev 11; R11 (P3 §7 step 6 stale — still said "retains failed entry only" / "dirty-only-failed pruning" / "single-POST assertion" contradicting R10's failed+gated retention and 2-POST retry contract; step 6 rewritten with "3-outcome pruning — retain failed+gated, prune only succeeded" + "exactly one demigrate + one migrate in correct order") → rev 12; R12 (P3 §8 history footer stale — still ended with "rev 11 (this commit)" and omitted R11 and R12 from the gate log; appended R11+R12 entries to restore audit-trail provenance) → rev 13; plan-R6 P2 (memo D3/D4 still described batched POSTs after the plan's R4 per-cell refactor; aligned memo D3 error-label + D4 applyChanges description + §7 step 4 with the per-cell POST model) → rev 14 (this commit).

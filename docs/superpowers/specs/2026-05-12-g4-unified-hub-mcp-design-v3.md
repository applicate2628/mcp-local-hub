...
6. **Session-client binding** — **required on every non-`initialize` JSON-RPC method and on DELETE; GET is exempt** (codex r7-bot-r2 P2 closure — earlier wording made the check conditional on header presence; codex r7-bot-r5 P2 closure — earlier wording also required Mcp-Session-Id on GET, which contradicts the MCP standards-fallback contract that GET returns 405 with `Allow: POST, DELETE` regardless of session state). The detailed rule:
    - **`initialize` (JSON-RPC over POST)**: `Mcp-Session-Id` header MUST be absent (initialize creates the session; presence → 400 "session-id only valid after initialize").
    - **Every non-`initialize` JSON-RPC method (over POST)**: header REQUIRED. Missing → 400 "Mcp-Session-Id required on non-initialize requests" with empty body. When present: session lookup must succeed AND `session.Client` must equal the path `client_id`. Unknown session id → 404 with `{"error":{"code":-32600,"message":"unknown session"}}`. Cross-client reuse → 401 with empty body (no oracle).
    - **`DELETE /clients/{id}/mcp`**: header REQUIRED. Idempotent termination — unknown session id → 404 (idempotent: re-issued DELETE after success also gets 404).
    - **`GET /clients/{id}/mcp`**: header is **NOT required and NOT checked**. The handler returns 405 with `Allow: POST, DELETE` and empty body unconditionally (modulo steps 1-5 of the auth gate, which still run — an unauthenticated GET still gets 401, but an authenticated GET without a session id gets 405 rather than 400). This preserves the MCP standards-fallback contract: compliant clients that probe via GET before they have a session id get the documented 405 rather than a confusing 400 or 401.

7. **MCP-Protocol-Version validation** (codex r7-bot P2 closure — required by MCP Streamable HTTP spec 2025-11-25). The MCP base spec mandates that every request AFTER `initialize` carry an `MCP-Protocol-Version` header naming the version the client agreed to during `initialize`. Hub checks: (a) header present on every method other than `initialize` AND `GET /clients/{id}/mcp` (codex r7-bot-r6 P2 closure — GET fallback is exempted because the MCP standards-fallback contract for GET is a method-level 405 returned BEFORE any version negotiation; a pre-session GET probe by definition has not yet completed initialize and cannot carry an agreed protocol version); (b) value matches the version stored in the session record at `initialize` time; (c) value is one of the versions the hub supports. `hubSupportedVersions = ["2025-11-25", "2025-06-18", "2025-03-26"]` (codex r7-bot-r5 P1 closure — earlier `["2025-11-25", "2025-06-18"]` would have broken existing in-repo callers that still emit `2025-03-26` during initialize, e.g. `internal/daemon/backend_lifecycle.go`, `internal/api/health.go`, `internal/api/register.go`). Add new versions as we test against new clients; remove old versions only after confirming no in-repo or known external caller still emits them. Missing header → 400 with empty body (NOT 401 — this is a protocol-level error, not an auth-level error). Mismatch → 400 with `{"error":{"code":-32600,"message":"protocol-version mismatch"}}` body. **`initialize` and `GET /clients/{id}/mcp` are both exempt from this header check** (initialize negotiates the version; GET is a method-level standards-fallback probe). All other requests must carry the header. The version-validation overall is NOT bypassed for `initialize` — see "initialize-time version rejection" below.

**Initialize-time version rejection** (codex r7-bot-r2 P2 closure — earlier wording let an unsupported `protocolVersion` pass initialize and only fail on the next request, creating a half-initialized session). Hub reads the client's offered `protocolVersion` from the JSON-RPC `initialize` params. If the offered version is NOT in `hubSupportedVersions`, hub responds with a JSON-RPC error result on the `initialize` itself: `{"jsonrpc":"2.0","id":<reqId>,"error":{"code":-32600,"message":"unsupported protocolVersion","data":{"offered":"<x>","supported":["2025-11-25","2025-06-18","2025-03-26"]}}}` — no session is created, no client_session_id is allocated. Clients get an immediate, actionable negotiation error rather than a delayed 400 on subsequent calls. If the offered version IS supported, hub stores it in `session.ProtocolVersion` and continues with the initialize fan-out.

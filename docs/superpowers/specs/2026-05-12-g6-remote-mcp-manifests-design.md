...
- For `transport: remote-http`:
  ...
  - `Command`, `BaseArgs`, `BaseArgsTemplate`, `Env`, `Daemons`, `PortPool`, `Languages`, `IdleTimeoutMin` are **REJECTED if set to a non-zero value** (codex bot P2 r1 on PR #152: silent ignore would let malformed manifests slip through). The Go zero values (empty string, nil slice, nil map, zero int, nil pointer) are accepted because they're indistinguishable from "field absent" in YAML; non-zero values cause `Validate` to fail with a clear message naming the offending field.
  - `WeeklyRefresh` is a `bool` — YAML cannot distinguish "absent" from "explicit `false`" once decoded into a Go bool (codex bot P2 r2 on PR #152). Special case: **accepted-but-no-op**. Setting `weekly_refresh: true` on a remote-http manifest emits a startup warning ("WeeklyRefresh has no effect on remote-http manifests; remove the line") but does not fail validation. `weekly_refresh: false` (the default) is silently accepted. This avoids breaking manifests that round-trip through the gui's `weekly_refresh: false` write path while still flagging the explicit-true case for operator attention.

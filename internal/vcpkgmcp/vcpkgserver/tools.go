	...
	if err := registerProjectableTool(vs, &mcp.Tool{
		...
		InputSchema: map[string]any{
			...
			"properties": map[string]any{
				...
				"max_depth":      map[string]any{"type": "integer", "minimum": 0, "maximum": cmakegraph.MaxDepthLimit, "description": "Optional traversal depth bound (per root)."},
				"max_nodes":      map[string]any{"type": "integer", "minimum": 0, "maximum": cmakegraph.MaxNodesLimit, "description": "Optional whole-tree node count bound (root ADMISSION)."},
				"max_file_bytes": map[string]any{"type": "integer", "minimum": 0, "maximum": cmakegraph.MaxFileBytesLimit, "description": "Optional per-file byte cap."},
				"max_roots":      map[string]any{"type": "integer", "minimum": 0, "maximum": cmakegraph.MaxRootsLimit, "description": "Root mode only. Optional bound on how many candidate roots are ENUMERATED before the tree walk stops (root_enumeration_capped). Separate from max_nodes, which bounds admission, not discovery."},
			},
		},

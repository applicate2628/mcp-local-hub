...
func (s *Server) groupConnection(name string, port int, hubLive bool, instanceID string, hasInstance bool) *groupConnectionDTO {
	...
	// Even when the hub is bound + primed, the aggregate listener can be wedged /
	// restarting / needing reconcile (Phase-0 item 1): do NOT hand out a live URL
	// the client would connect to a dead listener. Surface the degraded state as a
	// hint instead of advertising availability.
	if state, _ := s.hubHealth.snapshot(); hubHealthDegraded(state) {
		return &groupConnectionDTO{
			Available: false,

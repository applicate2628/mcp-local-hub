...
function AccessDeniedView(props: { env: SecretsEnvelope }) {
  return (
    <div class="banner banner-error" data-testid="vault-access-denied-banner">
      ...
      <p>If the refusal names the PARENT DIRECTORY (e.g. <code>%LOCALAPPDATA%\mcp-local-hub</code>) rather than a file, tighten the parent directory's permissions too (owner-only) — run <code>mcphub repair-state-dacl --path &lt;dir&gt;</code> on it, or set <code>MCPHUB_REQUIRE_SINGLE_USER_HOME=1</code> so mcphub enforces the strict parent-dir gate.</p>

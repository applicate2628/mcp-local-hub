...
func newKimiCodeCLIForTest(t *testing.T, initial string) *kimiCodeCLIClient {
	t.Helper()
	dir := t.TempDir()
	...
	return &kimiCodeCLIClient{jsonMCPClient: &jsonMCPClient{
		path:       path,
...
// TestKimiCodeCLI_AddEntry_WritesURLShape verifies AddEntry emits Kimi Code's
// minimal documented HTTP shape: just `url` (+ optional headers), with NO
// `type`/`transport`/`disabled` field (an entry with `url` and no `transport`
// is treated as an HTTP server), preserving unrelated top-level fields.
func TestKimiCodeCLI_AddEntry_WritesURLShape(t *testing.T) {
	cases := []struct {
		name    string
		entry   MCPEntry
		wantURL string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			k := newKimiCodeCLIForTest(t, `{"other":"keep-me"}`)
			...
			if _, has := entry["type"]; has {
				t.Errorf("type should be absent (Kimi Code infers HTTP from url): %v", entry)
			}
			if _, has := entry["transport"]; has {
				t.Errorf("transport should be absent (HTTP = url with no transport): %v", entry)
			}
			if _, has := entry["disabled"]; has {
				t.Errorf("disabled should be absent (not in Kimi Code docs shape): %v", entry)
			}
			if tc.wantHdr {
...
// TestKimiCodeCLI_GetEntry_ReadsURLField confirms GetEntry reads `url` and
// round-trips headers.
func TestKimiCodeCLI_GetEntry_ReadsURLField(t *testing.T) {
	k := newKimiCodeCLIForTest(t, `{
  "mcpServers": {
    "serena": {
...
// TestKimiCodeCLI_GetEntry_Missing returns nil for an absent entry.
func TestKimiCodeCLI_GetEntry_Missing(t *testing.T) {
	k := newKimiCodeCLIForTest(t, `{"mcpServers":{}}`)
...
// TestKimiCodeCLI_RemoveEntry_Inherited confirms RemoveEntry works and leaves
// other entries.
func TestKimiCodeCLI_RemoveEntry_Inherited(t *testing.T) {
	k := newKimiCodeCLIForTest(t, `{"mcpServers":{"serena":{"url":"http://x"},"other":{"url":"http://y"}}}`)
...
// TestKimiCodeCLI_RestoreEntryFromBackup_RestoresStdio restores a pre-hub stdio
// entry over a hub-HTTP live entry.
func TestKimiCodeCLI_RestoreEntryFromBackup_RestoresStdio(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "mcp.json")
	...
	k := &kimiCodeCLIClient{jsonMCPClient: &jsonMCPClient{path: path, clientName: "kimi-code-cli", urlField: "url"}}
...
// TestKimiCodeCLI_RestoreEntryFromBackup_RefusesHubBackupEntry verifies the
// inherited url-keyed guard refuses an already-hub-managed backup entry.
func TestKimiCodeCLI_RestoreEntryFromBackup_RefusesHubBackupEntry(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "mcp.json")
	if err := os.WriteFile(path, []byte(
...
// TestKimiCodeCLI_RestoreEntryFromBackupForRollback_AllowsHubEntry verifies the
// rollback variant bypasses the guard.
func TestKimiCodeCLI_RestoreEntryFromBackupForRollback_AllowsHubEntry(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "mcp.json")
	if err := os.WriteFile(path, []byte(
...
// TestKimiCodeCLI_BackupEntryIsHubManaged exercises the inherited url-keyed
// predicate.
func TestKimiCodeCLI_BackupEntryIsHubManaged(t *testing.T) {
	cases := []struct {
		name   string
		backup string
		entry  string
...
// TestKimiCodeCLI_Exists_DirBased verifies the dir-based Exists override.
func TestKimiCodeCLI_Exists_DirBased(t *testing.T) {
	dir := t.TempDir()
	cfgDir := filepath.Join(dir, ".kimi-code")
...
// TestKimiCodeCLI_BackupKeep_SeedsFreshInstall verifies BackupKeep seeds the
// parent dir + stub on a fresh install.
func TestKimiCodeCLI_BackupKeep_SeedsFreshInstall(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".kimi-code", "mcp.json")
	k := &kimiCodeCLIClient{jsonMCPClient: &jsonMCPClient{path: path, clientName: "kimi-code-cli", urlField: "url"}}
...
// TestKimiCodeCLI_NameAndConfigPath confirms the identifier and the default
// ~/.kimi-code/mcp.json path.
func TestKimiCodeCLI_NameAndConfigPath(t *testing.T) {
	c, err := NewKimiCodeCLI()
	if err != nil {
		t.Fatalf("NewKimiCodeCLI: %v", err)
	}
	if c.Name() != "kimi-code-cli" {
		t.Errorf("Name() = %q, want kimi-code-cli", c.Name())
	}
	if c.IsRelayStdio() {
	...
	if filepath.Base(filepath.Dir(got)) != ".kimi-code" {
		t.Errorf("ConfigPath() parent = %q, want .kimi-code (got %q)", filepath.Base(filepath.Dir(got)), got)
	}
}

// TestKimiCodeCLI_ConfigPath_HonorsKimiCodeHomeEnv verifies the KIMI_CODE_HOME
// override relocates the config file to $KIMI_CODE_HOME/mcp.json.
func TestKimiCodeCLI_ConfigPath_HonorsKimiCodeHomeEnv(t *testing.T) {
	custom := t.TempDir()
	t.Setenv("KIMI_CODE_HOME", custom)
	got := defaultKimiCodeCLIConfigPath()
	want := filepath.Join(custom, "mcp.json")
	if got != want {
		t.Errorf("defaultKimiCodeCLIConfigPath() = %q, want %q", got, want)
	}
}

...
func newCodeBuddyForTest(t *testing.T, initial string) *codeBuddyClient {
	t.Helper()
	dir := t.TempDir()
	cbDir := filepath.Join(dir, ".codebuddy")
	if err := os.MkdirAll(cbDir, 0o755); err != nil {
		t.Fatal(err)
	}
	path := filepath.Join(cbDir, "mcp.json")
	if initial != "" {
		if err := os.WriteFile(path, []byte(initial), 0600); err != nil {
			t.Fatal(err)
		}
	}
	return &codeBuddyClient{jsonMCPClient: &jsonMCPClient{
		path:       path,
...
func TestCodeBuddy_Name(t *testing.T) {
	c := newCodeBuddyForTest(t, `{"mcpServers":{}}`)
	if got := c.Name(); got != "codebuddy" {
		t.Errorf("Name() = %q, want codebuddy", got)
	}
}

func TestCodeBuddy_IsRelayStdio(t *testing.T) {
	c := newCodeBuddyForTest(t, `{"mcpServers":{}}`)
	if c.IsRelayStdio() {
		t.Error("IsRelayStdio() = true, want false (codebuddy is URL-native HTTP)")
	}
}

// TestCodeBuddy_AddEntry_WritesHTTPShape verifies the documented
// `{"type":"http","url":...}` shape (no `disabled` flag), plus `headers` when
// provided. Sibling entries and unrelated top-level fields survive the merge.
func TestCodeBuddy_AddEntry_WritesHTTPShape(t *testing.T) {
	tests := []struct {
		name    string
		initial string
		entry   MCPEntry
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			c := newCodeBuddyForTest(t, tc.initial)
			...
			// CodeBuddy's documented http shape carries no `disabled` flag.
			if _, has := got["disabled"]; has {
				t.Errorf("unexpected 'disabled' flag for CodeBuddy http entry: %v", got)
			}
...
func TestCodeBuddy_GetEntry_RoundTrip(t *testing.T) {
	c := newCodeBuddyForTest(t, `{"mcpServers":{}}`)
...
func TestCodeBuddy_RemoveEntry_Idempotent(t *testing.T) {
	c := newCodeBuddyForTest(t, `{"mcpServers":{"serena":{"type":"http","url":"http://localhost:9121/mcp"}}}`)
...
func TestCodeBuddy_InitEmpty(t *testing.T) {
	c := newCodeBuddyForTest(t, "")
...
func TestCodeBuddy_Exists_DirHeuristic(t *testing.T) {
	c := newCodeBuddyForTest(t, "")
	if !c.Exists() {
		t.Error("Exists() = false when ~/.codebuddy dir present, want true")
	}
}

func TestCodeBuddy_BackupKeep_FreshHost(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".codebuddy", "mcp.json")
	c := &codeBuddyClient{jsonMCPClient: &jsonMCPClient{
		path:       path,
		clientName: "codebuddy",
		urlField:   "url",
	}}
...
func TestCodeBuddy_DefaultConfigPath(t *testing.T) {
	got := defaultCodeBuddyConfigPath(filepath.Join("home", "u"))
	want := filepath.Join("home", "u", ".codebuddy", "mcp.json")
	if got != want {
		t.Errorf("defaultCodeBuddyConfigPath = %q, want %q", got, want)
	}
	c, err := NewCodeBuddy()
	if err != nil {
		t.Fatalf("NewCodeBuddy: %v", err)
	}
	if c.Name() != "codebuddy" {
		t.Errorf("Name() = %q, want codebuddy", c.Name())
	}
	if c.IsRelayStdio() {

...
func (v *vscodeClient) restoreEntryFromBackupWithWriter(backupPath, name string, allowHubEntry bool, writer WriteConfigFileFunc) error {
	...
	if err := v.restoreEntryFromBytes(backupData, name, allowHubEntry, writer); err != nil {
...
func (v *vscodeClient) restoreEntryFromBytes(backupData []byte, name string, allowHubEntry bool, writer WriteConfigFileFunc) error {
	...
	if allowHubEntry {
		...
		if handled, werr := wholeFileRestoreIfWriteTargetGone(v.path, backupData); handled {
			return werr
		}
		if liveMap, rerr := v.readJSON(); rerr == nil {
			liveServers, _ := liveMap[vscodeServersKey].(map[string]any)

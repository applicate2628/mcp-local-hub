...
func newDeepAgentsForTest(t *testing.T, initial string) *deepAgentsClient {
	t.Helper()
	dir := t.TempDir()
	daDir := filepath.Join(dir, ".deepagents")
	if err := os.MkdirAll(daDir, 0o755); err != nil {
		t.Fatal(err)
	}
	path := filepath.Join(daDir, ".mcp.json")
	if initial != "" {
		if err := os.WriteFile(path, []byte(initial), 0600); err != nil {
			t.Fatal(err)
		}
	}
	return &deepAgentsClient{jsonMCPClient: &jsonMCPClient{
		path:       path,
...
func TestDeepAgents_Name(t *testing.T) {
	c := newDeepAgentsForTest(t, `{"mcpServers":{}}`)
	if got := c.Name(); got != "deepagents" {
		t.Errorf("Name() = %q, want deepagents", got)
	}
}

func TestDeepAgents_IsRelayStdio(t *testing.T) {
	c := newDeepAgentsForTest(t, `{"mcpServers":{}}`)
	if c.IsRelayStdio() {
		t.Error("IsRelayStdio() = true, want false (deepagents is URL-native HTTP)")
	}
}

// TestDeepAgents_AddEntry_WritesHTTPShape verifies the documented
// `{"type":"http","url":...}` shape (no `disabled`), plus `headers` when
// provided. Siblings/top-level fields survive the merge.
func TestDeepAgents_AddEntry_WritesHTTPShape(t *testing.T) {
	tests := []struct {
		name    string
		initial string
		entry   MCPEntry
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			c := newDeepAgentsForTest(t, tc.initial)
			...
			if _, has := got["disabled"]; has {
				t.Errorf("unexpected 'disabled' flag for Deep Agents http entry: %v", got)
			}
...
func TestDeepAgents_GetEntry_RoundTrip(t *testing.T) {
	c := newDeepAgentsForTest(t, `{"mcpServers":{}}`)
...
func TestDeepAgents_RemoveEntry_Idempotent(t *testing.T) {
	c := newDeepAgentsForTest(t, `{"mcpServers":{"serena":{"type":"http","url":"http://localhost:9121/mcp"}}}`)
...
func TestDeepAgents_InitEmpty(t *testing.T) {
	c := newDeepAgentsForTest(t, "")
...
func TestDeepAgents_Exists_DirHeuristic(t *testing.T) {
	c := newDeepAgentsForTest(t, "")
	if !c.Exists() {
		t.Error("Exists() = false when ~/.deepagents dir present, want true")
	}
}

func TestDeepAgents_BackupKeep_FreshHost(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".deepagents", ".mcp.json")
	c := &deepAgentsClient{jsonMCPClient: &jsonMCPClient{
		path:       path,
		clientName: "deepagents",
		urlField:   "url",
	}}
...
func TestDeepAgents_DefaultConfigPath(t *testing.T) {
	got := defaultDeepAgentsConfigPath(filepath.Join("home", "u"))
	want := filepath.Join("home", "u", ".deepagents", ".mcp.json")
	if got != want {
		t.Errorf("defaultDeepAgentsConfigPath = %q, want %q", got, want)
	}
	c, err := NewDeepAgents()
	if err != nil {
		t.Fatalf("NewDeepAgents: %v", err)
	}
	if c.Name() != "deepagents" {
		t.Errorf("Name() = %q, want deepagents", c.Name())
	}
	if c.IsRelayStdio() {

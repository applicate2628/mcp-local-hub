...
func newCortexForTest(t *testing.T, initial string) *cortexClient {
	t.Helper()
	dir := t.TempDir()
	cortexDir := filepath.Join(dir, ".snowflake", "cortex")
	if err := os.MkdirAll(cortexDir, 0o755); err != nil {
		t.Fatal(err)
	}
	path := filepath.Join(cortexDir, "mcp.json")
	if initial != "" {
		if err := os.WriteFile(path, []byte(initial), 0600); err != nil {
			t.Fatal(err)
		}
	}
	return &cortexClient{jsonMCPClient: &jsonMCPClient{
		path:       path,
		clientName: "cortex",
		urlField:   "url",
	}}
...
func TestCortex_Name(t *testing.T) {
	c := newCortexForTest(t, `{"mcpServers":{}}`)
	if got := c.Name(); got != "cortex" {
		t.Errorf("Name() = %q, want cortex", got)
	}
}

func TestCortex_IsRelayStdio(t *testing.T) {
	c := newCortexForTest(t, `{"mcpServers":{}}`)
	if c.IsRelayStdio() {
		t.Error("IsRelayStdio() = true, want false (cortex is URL-native HTTP)")
	}
}

// TestCortex_AddEntry_WritesHTTPShape verifies the documented
// `{"type":"http","url":...}` shape (no `disabled`), plus `headers` when
// provided. Siblings/top-level fields survive the merge.
func TestCortex_AddEntry_WritesHTTPShape(t *testing.T) {
	tests := []struct {
		name    string
		initial string
		entry   MCPEntry
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			c := newCortexForTest(t, tc.initial)
			...
			if _, has := got["disabled"]; has {
				t.Errorf("unexpected 'disabled' flag for Cortex http entry: %v", got)
			}
...
func TestCortex_GetEntry_RoundTrip(t *testing.T) {
	c := newCortexForTest(t, `{"mcpServers":{}}`)
...
func TestCortex_RemoveEntry_Idempotent(t *testing.T) {
	c := newCortexForTest(t, `{"mcpServers":{"serena":{"type":"http","url":"http://localhost:9121/mcp"}}}`)
...
func TestCortex_InitEmpty(t *testing.T) {
	c := newCortexForTest(t, "")
...
func TestCortex_Exists_DirHeuristic(t *testing.T) {
	c := newCortexForTest(t, "")
	if !c.Exists() {
		t.Error("Exists() = false when ~/.snowflake/cortex dir present, want true")
	}
}

// TestCortex_BackupKeep_FreshHost verifies BackupKeep creates the two-level
// parent dir and seeds a stub on a clean host.
func TestCortex_BackupKeep_FreshHost(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".snowflake", "cortex", "mcp.json")
	c := &cortexClient{jsonMCPClient: &jsonMCPClient{
		path:       path,
		clientName: "cortex",
		urlField:   "url",
	}}
...
func TestCortex_DefaultConfigPath(t *testing.T) {
	got := defaultCortexConfigPath(filepath.Join("home", "u"))
	want := filepath.Join("home", "u", ".snowflake", "cortex", "mcp.json")
	if got != want {
		t.Errorf("defaultCortexConfigPath = %q, want %q", got, want)
	}
	c, err := NewCortex()
	if err != nil {
	...
	if c.Name() != "cortex" {
		t.Errorf("Name() = %q, want cortex", c.Name())
	}
	if c.IsRelayStdio() {

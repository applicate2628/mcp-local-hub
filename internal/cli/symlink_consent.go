...
func installInteractiveSymlinkConsent(out io.Writer, in *os.File) (restore func()) {
	...
	api.InteractiveSymlinkConsent = func(client, originalPath, pinnedPath string) bool {
		return promptInteractiveSymlinkConsent(out, reader, client, originalPath, pinnedPath)
	}
...
// it is unit-testable with an in-memory reader (no real TTY). `pinnedPath` is
// the FULL resolved real target path (parent + file) the write is pinned to —
// shown to the operator so they consent to a concrete file, not an abstract
// "follow".
func promptInteractiveSymlinkConsent(out io.Writer, in *bufio.Reader, client, originalPath, pinnedPath string) bool {
	...
	fmt.Fprintf(out,
		"%s config %s is a symlink -> real target is %s. Write there? [y/N] ",
		subject, originalPath, pinnedPath)

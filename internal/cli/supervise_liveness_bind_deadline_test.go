...
func TestSweep_PortUnboundWithinStartupDeadlineNoRestart(t *testing.T) {
	...
	sweepSupervisorLivenessOnce(f.stateDir, f.intent, f.tracker, f.loop, nil, map[string]int{})
...
func TestSweep_PortUnboundPastStartupDeadlineRestartsAndEmitsBindTimeout(t *testing.T) {
	...
	sweepSupervisorLivenessOnce(stateDir, intent, tracker, loop, auditLog, map[string]int{})
...
func TestSweep_PostFirstBindUnboundRestartsAt5s(t *testing.T) {
	...
	sweepSupervisorLivenessOnce(f.stateDir, f.intent, f.tracker, f.loop, nil, latch)
	restore1()

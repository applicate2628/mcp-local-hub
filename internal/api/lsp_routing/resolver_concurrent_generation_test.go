// internal/api/lsp_routing/resolver_concurrent_generation_test.go
//
// LSP twin of serena_routing's
// TestNewReadOnlyWorkspaceResolver_ConcurrentRefreshesCannotRegress — see
// that file's package doc comment for the full A2 mechanism
// (architecture-adversarial-reverify.md + qa-adversarial-falsifiers.md,
// work-items/active/2026-07-25-mcp-front-daemon/). This resolver duplicates
// the same stat/load/cache algorithm and carries the identical fix
// (refreshMu + refreshLoadedHookForTest), so it needs the identical
// falsifying test.
package lsp_routing
...
// A2 falsifying test for the LSP resolver.
//
...
func TestNewReadOnlyWorkspaceResolver_ConcurrentRefreshesCannotRegress(t *testing.T) {
	root := t.TempDir()
	wsPath := filepath.Join(root, "go-project")
	writeFile(t, filepath.Join(wsPath, "go.mod"))
	...
	writeGeneration := func(port int, mtime time.Time) {
		...
		if err := g.PutLSP(api.WorkspaceEntry{
			WorkspaceKey:  api.WorkspaceKey(wsPath),
			WorkspacePath: wsPath,
			Language:      "go",
			Backend:       "gopls-mcp",
			Port:          port,
			TaskName:      "mcp-local-hub-lsp-concurrent",
		}); err != nil {
			t.Fatalf("PutLSP port %d: %v", port, err)
		}
		if err := g.Save(); err != nil {
			t.Fatalf("Save port %d: %v", port, err)
		}
		if err := os.Chtimes(regPath, mtime, mtime); err != nil {
			t.Fatalf("Chtimes port %d: %v", port, err)
		}
	}
	...
	if entries := resolver.snapshotRegistry().Workspaces; len(entries) != 1 || entries[0].Port != portSeed {
	...
	go func() {
		defer wg.Done()
		resolver.snapshotRegistry() // pauses inside refreshLoadedHookForTest above
	}()
	...
	// snapshotRegistry()/refresh() call, which would itself trigger a
	// further reload (the file's current mtime may still differ from
	// whatever the buggy pre-fix code last published) and could self-heal a
	// transient regression that already happened, hiding the very bug this

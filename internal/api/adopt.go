...
func (a *API) ExecuteAdoptWithOpts(plan *AdoptPlan, w io.Writer, opts ExecuteAdoptOpts) error {
	...
	transactionErr := func() error {
		...
		stage = "install"
		if err := a.installWithAdoptLeaseHeld(context.Background(), installOpts, nil); err != nil {
			...
			if plan.providerSource != nil {
				// Do not let ordinary-error cleanup re-enable a direct provider
				// beside committed/uncertain intent. Reuse the guarded recovery
				...
				_, recoveryErr := a.executeProviderRecoveryDeAdopt(rec, ExecuteDeAdoptOpts{providerDeps: opts.providerDeps}, io.Discard)
				return fmt.Errorf("adopt install failed; provider cleanup used durable recovery: %w", errors.Join(err, recoveryErr))
			}

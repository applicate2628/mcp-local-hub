...
// rationale.
//
// The stub starts cmd via cmd.Start() and returns the spawned PID so
// callers (notably the v0.5.0 supervisor reconcile loop) can stay
// platform-agnostic at the call site. The job argument is accepted
// but ignored — the POSIX Job is itself a no-op stub.
//

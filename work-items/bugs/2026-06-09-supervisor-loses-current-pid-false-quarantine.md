...
2. At some later point the supervisor's `current_pid` for time was reset to 0 while
   ...
   never closed → the child was never reaped). **Trigger for the current_pid reset
   is not yet pinned** (needs supervisor state-history forensics — candidate causes:
   a reconcile path that zeroes current_pid, a partial state write, or an in-memory
   tracking loss without a corresponding child reap).
...
- **Layer A (trigger, unconfirmed):** what reset `current_pid` to 0 for a live
  child. Must be reproduced / traced before claiming a root-cause fix (no kostyl).
- **Layer B (the amplifier, confirmed):** the respawn path, on
...
- **Layer B fix (defensive, high value):** before respawning a daemon whose
...
- **Layer A fix:** trace and eliminate the current_pid reset (or, if it is an
  expected transient, ensure the adopt path in Layer B covers it).
...
Follow-up after #268 merges + deploys. The #268 supervisor restart fixes the
immediate `time` (and `memory`) instances regardless; the Layer A/B fix is a
separate focused change so it does not expand #268 mid-review.

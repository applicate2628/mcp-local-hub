...
## Known test-debt (не блокер петли)

Полный `go test ./internal/api/` имеет order-dependent загрязнение: ~5-7
тестов (TestInstallAll*, TestUnregister_*, TestDialSupervisorIPC*) падают
ТОЛЬКО в полном прогоне, чисты соло/группами. Плюс env-классы (живой флот:
порты, пайп). Рабочий протокол — narrow -run + соло-подтверждение. Чистка
сеамов — отдельный пост-PASS пункт.

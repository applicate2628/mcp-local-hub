...
**Rate limit** (sweep-local): ≤1 identity lookup/30s and ≤3 reap attempts per failure window per task; beyond → downgrade to observe-only with `rate_limited: true` pointing at the recover verb.

...
partially-settles: work-items/backlog/closed/2026-06-25-p3b-reenable-value-source.md (security/value-source question settled here; D2 cold-re-enable UX subsequently RESOLVED by #439 — embed-only catalog-by-name prefill replaced the bare #/add-server CTA)
...
## Deferrals from P3b v1 (D1 is a permanent/separate deferral; D2 was RESOLVED by #439)
D1 claude Local-scope toggle (no write owner; SHIPPED read-only — `Projects.tsx` Local subsection is read-only; permanent/separate deferral, the catastrophic `~/.claude.json` corruption surface keeps write-back out of v1) · D2 object-member cold re-enable via toggle — RESOLVED by #439 (live 5fbef07c): the bare `#/add-server` CTA was replaced with embed-only catalog-by-name prefill + shipped-server edit-shadow notice + clobber-guard; tracked in `work-items/backlog/closed/2026-06-25-p3b-reenable-value-source.md` (the deeper stash-full-restore remains deferred in `work-items/backlog/2026-06-27-d2-stash-full-restore-deferred.md`) · D3 group↔project binding filter (shipped in P3c #435) · D4 project-scoped Servers view (would touch protected Servers.tsx — not built).

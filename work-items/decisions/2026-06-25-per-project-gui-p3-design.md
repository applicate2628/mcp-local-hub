...
Implemented: P3a (#433) + P3b (#434) + P3c (#435) SHIPPED, live 8c359a1c; D2 cold object-member re-enable subsequently RESOLVED by #439 (live 5fbef07c). Remaining residual: D1 (claude Local-scope toggle — shipped READ-ONLY in v1; the GUI renders `~/.claude.json` Local servers read-only; permanent/separate deferral). D2 (cold object-member re-enable via the per-row toggle) was resolved by #439 (embed-only catalog-by-name prefill replaced the bare `#/add-server` CTA) — tracked in `work-items/backlog/closed/2026-06-25-p3b-reenable-value-source.md`; the deeper stash-full-restore remains deferred in `work-items/backlog/2026-06-27-d2-stash-full-restore-deferred.md`. A latent backend gap was tracked in `work-items/bugs/closed/2026-06-27-claude-code-object-member-destructive-api-path.md`. The planning-tense dependency/blocker notes below are HISTORICAL (design provenance, not live gates).
...
> guarantee. Tracked: `work-items/bugs/closed/2026-06-27-claude-code-object-member-destructive-api-path.md`.
>

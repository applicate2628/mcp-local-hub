...
6. A guard that FAILS TO EVALUATE counts as an unavailable proof and spares the candidate. Every gate must be written so that an error raised while computing it (a malformed pattern, a failed attribute read, a permission error on the target, an exception inside the predicate) returns "refuse", never falls through to the next statement. Enforcement: each gate returns an explicit `(allow | refuse, reason)` verdict rather than a bare boolean, the apply path treats any non-`allow` — including an error verdict — as `refuse`, and a test injects a throwing gate to prove no termination occurs. Evidence for the rule: on 2026-07-09 an operator-side reap script's image-path guard threw on a malformed pattern; because the error was non-terminating, execution continued to the terminate call. The kill happened to be correct (identity had been verified out of band), but the guard that was supposed to authorize it never ran. A guard whose failure mode is to continue is more dangerous than no guard, because it reads as fail-closed and is not.

Preview-only diagnostics may include PID, started-at, redacted or hashed executable path, basename or branch pattern class, argv shape, age, and owner when available. Required refusal reasons include `env-read-error`, `env-override-absent`, `basename-not-in-branch`, `requires-explicit-temp-root`, and `guard-evaluation-error`.

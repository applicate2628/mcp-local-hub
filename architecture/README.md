...
**Статус:** начальный baseline создан штатным генератором из чистого коммита `1b875e3ad9f0ca7d4bc306edb6c22431b66da6c7` и содержит 2412 записей. Строгая проверка, обычные тесты, race с перемешиванием и тремя повторами, а также vet прошли. Это подтверждает архитектурный baseline, но не заменяет проверки установленного продукта и разрешение на публикацию.
...
Полный состав обязательств A2 определяется [каноническим планом](../docs/modernization/plans/wp-11a-to-11f.md#wp-11a). Общий помощник для сравнения эталонных результатов A3/A4 находится в `internal/testkit`; он не является отдельным инструментом или генератором.
...
| [golden.go](../internal/testkit/golden.go) | Общее сравнение эталонного JSON для тестов A3/A4; файлы только читаются |
...
Для эталонных тестов используется `testkit.AssertJSONGolden(t, "testdata/scenario.json", normalized)`. Эталон хранится в формате `encoding/json.MarshalIndent` с отступом в два пробела и завершающим переводом строки. Помощник не обновляет файлы автоматически и не сортирует массивы. Нормализация времени, идентификаторов процессов и путей принадлежит вызывающему тесту; большие числа и различие `null`/пустого массива сохраняются.
...
Принятые [решением D-013](../docs/modernization/decisions.md#d-013) ограничения размера файлов: 1000 строк для предупреждения о рабочем коде, 1500 строк для жёсткого порога рабочего кода, 2000 строк для тестового файла. Initial inventory выявила 77 существующих нарушений; они остаются долгом baseline, а финальная генерация выполняется повторно на зафиксированном исходном коммите.

<a id="inventory"></a>
## 3. Инвентаризация и назначение владельцев

Первый проход initial inventory с принятыми порогами выявил 77 существующих file-budget нарушений: 31 production advisory, 24 production hard и 22 test review. Это срез до финального source freeze, а не доказательство финального baseline. После коммита всех входов scan и baseline повторяются на точном `scan_sha`.

Первый проход нужен для исследования: сохранить отчёт `archcheck scan`, изучить распределение размеров файлов и назначить реальные владельцы всем найденным нарушениям. Каждый baseline entry должен соответствовать specific-first правилу владельца; временный catch-all не используется.
...
go test ./internal/archguard ./tools/archcheck ./internal/testkit -count=1 \
...
go test -race -shuffle=on ./internal/archguard ./tools/archcheck ./internal/testkit -count=3 \
  > "$reports/race-shuffle.log" 2>&1
go vet ./internal/archguard ./tools/archcheck ./internal/testkit \
  > "$reports/vet.log" 2>&1
...
3. Ограничения размеров файлов D-013 подтверждены initial inventory; финальный baseline повторно создан из зафиксированного исходного коммита.
4. Полные тесты `internal/archguard`, `tools/archcheck` и `internal/testkit`, обнаружение гонок, перемешивание порядка тестов, `go vet`, сборка анализатора и неотфильтрованный `verify` проходят.

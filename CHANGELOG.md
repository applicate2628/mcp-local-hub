...
- `CSRF` — Cross-Site Request Forgery; a browser-driven attack class.
- `DNS-rebind` / `DNS-rebinding` — an attack that binds an attacker hostname to `127.0.0.1`; defended by the GUI Host-allowlist.
- `MCP` — Model Context Protocol; the protocol this hub multiplexes.
- `pidport` — the per-instance lock+metadata file that records a running `mcphub gui` process's PID and bound port.
- `PR` — Pull Request.
- `REVISE bundle` — PR `#128`, a single PR that bundles 13 corrected versions of bot-flagged fixes.
- `SSE` — Server-Sent Events; long-lived HTTP event stream.
- `S1`–`S4` — section labels in the 2026-05-04 security audit (PR `#51`); see `SECURITY.md` Terms.
